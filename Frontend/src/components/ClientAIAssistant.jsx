...
import { CLIENT_AI_FEATURES, getClientInsight } from "../services/aiService";

/* ======================================================
   ICON MAP (feature registry -> lucide component)
====================================================== */
const ICONS = { FileText, Gauge, MessageSquare, CalendarClock, TrendingUp };
  ...
  lines.forEach((raw, idx) => {
    ...
    if (heading) {
      ...
      blocks.push(
        <p
          key={`h-${idx}`}
          className={`${sizes[level] || sizes[3]} text-slate-900`}
        >
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm font-medium text-indigo-600">
        ...
        Analyzing client with Claude AI
...
/* ======================================================
   MAIN COMPONENT
====================================================== */
export default function ClientAIAssistant({ client, onClose }) {
  ...
  const run = useCallback(
    async (type) => {
      ...
      setCopied(false);
      try {
        const data = await getClientInsight(type, client);
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/80">Insights for {clientName}</p>
              </div>
            </div>
            <button
              onClick={onClose}
