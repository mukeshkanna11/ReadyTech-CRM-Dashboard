...
import { Sparkles, PenTool, Copy, Check, RefreshCw, Wand2, AlertTriangle } from "lucide-react";
...
const TONES = ["Professional", "Friendly", "Persuasive", "Casual", "Enthusiastic", "Formal"];

const buildPrompt = (type, topic, tone) =>
  `Write a ${tone.toLowerCase()} ${type} about "${topic}".
Return clean, well-structured content ready to publish. Use GitHub-flavored Markdown where helpful.`;
...
export default function AIContentGenerator() {
  ...
  const generate = async () => {
    ...
    setCopied(false);
    try {
      const res = await API.post("/ai/chat", { prompt: buildPrompt(type, topic, tone) });
      ...
      <div className="relative overflow-hidden text-white shadow-xl rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-900">
        ...
        <div className="relative flex items-center gap-4 p-6 sm:p-8">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">AI Content Generator</h1>
            <p className="mt-1 text-sm text-slate-300">
              Generate on-brand copy for blogs, social, email and products in seconds.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="p-6 space-y-5 bg-white border shadow-sm rounded-3xl border-slate-200 lg:col-span-2">
          ...
          <Field label="Topic">
            <textarea
              ...
              rows={3}
              placeholder="e.g. Benefits of a cloud-based CRM for small businesses"
        ...
        <div className="p-6 bg-white border shadow-sm rounded-3xl border-slate-200 lg:col-span-3">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-slate-900">Generated Content</h3>
            {result && !loading && (
              <button
                onClick={copy}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h4 className="text-sm font-semibold text-slate-700">Nothing generated yet</h4>
              <p className="max-w-xs mt-1 text-xs text-slate-400">
                Pick a content type, add a topic and tone, then hit Generate.
              </p>
            </div>
          )}
        </div>
